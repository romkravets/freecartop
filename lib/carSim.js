...
import { getDepreciation } from './carMarket.js';
...
export function buildProjection(profile) {
  const { make, model, importCountry, fuel, year, mileage, price, transmission } = profile;
  const initHealth = calcInitialHealth(year, mileage, importCountry, transmission);
  const fixedCosts = calcFixedCosts(fuel, importCountry);
  const fixedTotal = fixedCosts.reduce((s, c) => s + c.amount, 0);

  // Use model-specific depreciation rates when available (more realistic than make-level)
  const modelDepRates = getDepreciation(make, model ?? '');
  const imp      = IMPORT_MODIFIER[importCountry] ?? IMPORT_MODIFIER.ua;
  const makeData = getMakeData(make);
  ...
  for (let y = 0; y <= SIM_YEARS; y++) {
    ...
    if (y < SIM_YEARS) {
      // Model-specific rate takes priority over generic make rate
      let rate;
      if (modelDepRates) {
        rate = modelDepRates[y] + imp.depExtra;
        if (fuel === 'diesel') rate += 0.01;
      } else {
        rate = makeData.dep + imp.depExtra;
        if (fuel === 'diesel') rate += 0.01;
        if (y >= 3) rate = Math.max(0.04, rate - 0.02);
      }
      curPrice = Math.max(500, Math.round(curPrice * (1 - rate)));
