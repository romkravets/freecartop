...
export const IMPORT_OPTIONS = [
  ...
  { value: "de", label: "🇩🇪 Німеччина", riskNote: null },
  {
    value: "usa",
    label: "🇺🇸 США",
    riskNote: "ризик ДТП/крадіжки (CARFAX обов'язково)",
  },
...
export function analyzeRisk(input) {
  ...
  if (prevs.length > 0) {
    ...
    if (maxPrev > claimedMileage) {
      ...
      const cut =
        diff > 100_000 ? 65 : diff > 50_000 ? 55 : diff > 20_000 ? 45 : 35;
      score -= cut;
      flags.push({
        severity: "critical",
        icon: "🔢",
      ...
      positives.push({
        icon: "✅",
        text: `Попередній пробіг (${maxPrev.toLocaleString("uk-UA")} км) підтверджує поточний`,
      });
    }
  }
  ...
  if (claimedMileage > 0 && carAge >= 3) {
    ...
    if (ratio < 0.25 && prevs.length === 0) {
      ...
      positives.push({
        icon: "✅",
        text: `Пробіг відповідає віку (${claimedMileage.toLocaleString("uk-UA")} км / ${carAge} р.)`,
      });
    }
  }
  ...
  if (owners >= 4) {
    ...
    flags.push({
      ...
      detail:
        'Часта зміна власників = приховані проблеми або "перекупство". Перевіряй кожного',
    });
  } else if (owners === 3) {
    score -= 15;
    flags.push({
  ...
  if (fastReReg) {
    ...
    flags.push({
      ...
      detail:
        "Власник намагався якнайшвидше позбутись авто — що він знав? Дуже погана ознака",
    });
  }
  ...
  if (importCountry === "usa") {
    ...
    flags.push({
      ...
      title: "Авто з США",
      detail:
        "Ризик прихованих пошкоджень після ДТП або крадіжки. Обов'язково перевір CARFAX / AutoCheck перед переговорами",
    });
    ...
    positives.push({
      icon: "✅",
      text: `Авто з ${importCountry === "de" ? "Німеччини" : "ЄС"} — хороша ознака`,
    });
  } else {
  ...
  if (weeksOnSale >= 104) {
    ...
    flags.push({
      ...
      detail:
        "Ринок відмовляється від цього авто. Задайся питанням: чому ніхто не купив за 2 роки?",
    });
    ...
    flags.push({
      ...
      detail:
        "Довго не може знайти покупця. Є привід для серйозного торгу або сховані проблеми",
    });
  ...
  if (marketPrice > 0 && priceAsked > 0) {
    ...
    if (pct > 0.3) {
      ...
      positives.push({
        icon: "✅",
        text: `Ціна ринкова (~$${marketPrice.toLocaleString()})`,
      });
    }
  }
  ...
  if (vinChecked) {
    ...
    flags.push({
      ...
      detail:
        "Перевір на carvertical.com, auto.ria.com або carfax.com перед зустріччю",
    });
  }
...
export function generateShareText(carInfo, result, marketPrice) {
  ...
  if (marketPrice > 0 && priceAsked > 0) {
    ...
    if (Math.abs(diff) > 5) {
      ...
      lines.push(
        `💰 Ціна: $${priceAsked.toLocaleString()} (~ринок $${marketPrice.toLocaleString()})`,
      );
    }
  }
